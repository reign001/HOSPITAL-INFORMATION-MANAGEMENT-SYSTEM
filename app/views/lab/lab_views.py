...
def lab_requests(patient_id):
    ...
    # Fetch **all requests** for this patient, newest first
    all_requests = LabRequest.query.filter_by(patient_id=patient.id)\
                                   .order_by(LabRequest.created_at.desc()).all()

    # Determine the latest request, if any
    latest_lab_request = all_requests[0] if all_requests else None

    if request.method == "POST":
        tests = request.form.get("tests")

        # 1️⃣ Try to find an existing visit
        visit = LabVisit.query.filter_by(patient_name=patient.first_name)\
                              .order_by(LabVisit.created_at.desc()).first()

        # 2️⃣ Create a visit if none exists
        if not visit:
            visit = LabVisit(
                patient_name=patient.first_name,
                patient_id=patient.id,
            ...
            db.session.add(visit)
            db.session.flush()  # ensures visit.id is available

        # 3️⃣ Create the lab request linked to the visit
        ...
        return redirect(url_for("lab.lab_detail", visit_id=visit.id))

    return render_template(
        "lab/request.html",
        patient=patient,
        latest_lab_request=latest_lab_request,  # ✅ Pass this to the template
        all_requests=all_requests               # optional if you want to show history
    )
...
def process_lab_request(request_id):
    ...
    if not lab_request.visit_id:
        ...
        visit = LabVisit(
            ...
            patient_name=patient.first_name,
            patient_surname=patient.surname,
            patient_age=patient.calculate_age() or 0,
    ...
    # GET -> show form
    return render_template("lab/process_lab_request.html", lab_request=lab_request)
...
def lab_results():
    # Fetch lab results for the nurse view
    results = LabRequest.query.filter_by(status="pending").all()
    return render_template("lab/request.html", results=results)